...
int DendrogramHardened::getClusterId(cluster_t *cluster_ptr) {
...
void
DendrogramHardened::splitPoints(int **points_1, int **points_2, int *points_1_size, int *points_2_size, int *points_membership,
...
int DendrogramHardened::areAllClustersInLevelUnitary(int level) {
...
int DendrogramHardened::getLevels() {
    clusters.size();
}

int DendrogramHardened::splitCluster(int level, int *points_membership, cluster_t *original_cluster) {
...
int DendrogramHardened::countClustersInLevel(int level) {
...
float **DendrogramHardened::getPointsInCluster(cluster_t *cluster, float **points, int n_features) {
...
void DendrogramHardened::printCluster(cluster_t *cluster) {
...
int DendrogramHardened::toTextFile(char *filename) {
...
int DendrogramHardened::toBinaryFile(char *filename) {
...
int DendrogramHardened::fromBinaryFile(char *filename) {
...
void DendrogramHardened::catalogIds() {
    auto dendroIt = clusters.begin();
    while (dendroIt != clusters.end()) {
        cluster_t *cluster = dendroIt->second;
        do {
...
void DendrogramHardened::updateIds() {
    //catalogIds();
...
void DendrogramHardened::printClusterSavedIds(cluster_t *cluster) {
    std::cout << "CLUSTER #" << getClusterId(cluster) << " {\n";
    std::cout << "\t\tpoints -> { ";
    for (int i = 0; i < cluster->size; ++i) {
        std::cout << cluster->points[i];
...
void DendrogramHardened::printWithSavedIds() {
    std::cout << "\n\nDENDROGRAM:\n";
...
void DendrogramHardened::printIdsMap() {
    std::cout << "\n\nIDS MAP: ";
...
DendrogramHardened::DendrogramHardened(char* binary_filename) {
    fromBinaryFile(binary_filename);
}

DendrogramHardened::~DendrogramHardened() {
    auto dendroIt = clusters.begin();
    while (dendroIt != clusters.end()) {
        cluster_t *cluster = dendroIt->second;
        do {
